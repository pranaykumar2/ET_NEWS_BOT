...
class EnhancedNewsImageGenerator:
    ...
    def __init__(self, 
        ...
        self.font_tag_style = getattr(Config, 'FONT_TAG_STYLE', '800')
        ...
        self.title_line_height = getattr(Config, 'TITLE_LINE_HEIGHT', 50)
        self.description_line_height = getattr(Config, 'DESCRIPTION_LINE_HEIGHT', 40)
    ...
    def generate_news_image(self, title, description="", timestamp="", image_data: Optional[bytes] = None):
        ...
        Args:
            ...
            image_data: Optional news image data (bytes)
        ...
        title_lines = self.wrap_text(title, self.font_title, text_area_width if image_data else int(text_area_width * 0.85))
        title_height = len(title_lines) * self.title_line_height
        
        # Calculate description height - wrap ALL lines, no limit
        desc_height = 0
        if description:
            desc_lines = self.wrap_text(description, self.font_body, text_area_width if image_data else int(text_area_width * 0.7))
        ...
        if image_data:
            ...
            news_img = self.process_image_data(image_data)
            if news_img:
                img_x = text_width + 40
